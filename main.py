...
# end of Assignment8 class definition
...
infile = open("Customers and Restaurants 2.csv", "r")
# read the first line : the restaurant names
restaurants = infile.readline().split(",")

# print(restaurants)

# start at 1 because the first element is blank
for r in restaurants[1:]:
    # create all the Restaurant objects
    rest_list.append(Restaurant(name=r.strip()))

# read the rest of the lines
for line in infile:
    line_list = line.split(",")
    # the first element on the line is the customer name
    new_customer = Customer(line_list[0].strip())
    cust_list.append(new_customer)
    # the rest of the line is all 0's and 1's
    for i in range(1, len(line_list)):
        c = int(line_list[i].strip())
        # if c == 1, add both the connections:
        if c == 1:
            # add the Restaurant to the Customer's connections
            new_customer.add_connection(rest_list[i-1])
            # add the Customer to the Restaurant's connections
            rest_list[i-1].add_connection(new_customer)
